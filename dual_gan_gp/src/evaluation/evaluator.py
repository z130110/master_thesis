...
class Evaluator(object):
    ...
    def __init__(self, trainer):
        ...
        self.discriminator_1 = trainer.discriminator_1
        self.discriminator_2 = trainer.discriminator_2
    ...
    def eval_dis(self, to_log):
        ...
        for i in range(0, self.src_emb.num_embeddings, bs):
            ...
            preds_d1s = self.discriminator_1(self.mapping(emb))
            src_preds_d1.extend(preds_d1s.data.cpu().tolist())


        for i in range(0, self.src_emb.num_embeddings, bs):
            emb = Variable(self.src_emb.weight[i:i + bs].data, volatile=True)
            preds_d2s = self.discriminator_2(self.mapping(emb))
            src_preds_d2.extend(preds_d2s.data.cpu().tolist())


        for i in range(0, self.tgt_emb.num_embeddings, bs):
            emb = Variable(self.tgt_emb.weight[i:i + bs].data, volatile=True)
            preds_d1t = self.discriminator_1(emb)
            tgt_preds_d1.extend(preds_d1t.data.cpu().tolist())

        for i in range(0, self.tgt_emb.num_embeddings, bs):
            emb = Variable(self.tgt_emb.weight[i:i + bs].data, volatile=True)
            preds_d2t = self.discriminator_2(emb)
            tgt_preds_d2.extend(preds_d2t.data.cpu().tolist())


        src_pred_d1 = np.mean(src_preds_d1)
        src_pred_d2 = np.mean(src_preds_d2)
        tgt_pred_d1 = np.mean(tgt_preds_d1)
        tgt_pred_d2 = np.mean(tgt_preds_d2)
        logger.info("D1 source pred:%.5f, D1 target pred:%.5f, D2 source pred:%.5f, D2 source pred:%.5f,"
                    % (src_pred_d1, tgt_pred_d1, src_pred_d2, tgt_pred_d2))

        src_accu_d1 = np.mean([x >= 0.5 for x in src_preds_d1])
        src_accu_d2 = np.mean([x >= 0.5 for x in src_preds_d2])
        tgt_accu_d1 = np.mean([x < 0.5 for x in tgt_preds_d1])
        tgt_accu_d2 = np.mean([x < 0.5 for x in tgt_preds_d2])
        tem_src_d1 = src_accu_d1 * self.src_emb.num_embeddings
        tem_src_d2 = src_accu_d2 * self.src_emb.num_embeddings
        tem_tgt_d1 = tgt_accu_d1 * self.tgt_emb.num_embeddings
        tem_tgt_d2 = tgt_accu_d2 * self.src_emb.num_embeddings
        tem_total_emb = 2*self.src_emb.num_embeddings + 2*self.tgt_emb.num_embeddings

        dis_accu = ((tem_src_d1 + tem_src_d2 + tem_tgt_d1 + tem_tgt_d2) /
                    (tem_total_emb))
        logger.info("Accuracies: D1 source:%.5f, D1 target:%.5f, D2 source:%.5f, D2 target:%.5f, global:%.5f"
                    % (src_accu_d1, tgt_accu_d1, src_accu_d2, tgt_accu_d2, dis_accu))

        to_log['dis_accu'] = dis_accu
        to_log['dis_1_src_pred'] = src_pred_d1
        to_log['dis_2_src_pred'] = src_pred_d2
        to_log['dis_2_tgt_pred'] = tgt_pred_d1
        to_log['dis_2_tgt_pred'] = tgt_pred_d2
